    ...
    stages {
        ...
        stage('Testes'){      
			...
			withCredentials([file(credentialsId: "sgp_robot_resource", variable: 'resource'), file(credentialsId: "sgp_robot_filter", variable: 'filter')]){
			  ...
			  sh('docker run --rm -v $(pwd)/testes/scripts:/opt/robotframework/tests:Z -v $(pwd)/testes/resultados:/opt/robotframework/reports:Z -e BROWSER=firefox -e TZ=America/Sao_Paulo ppodgorsek/robot-framework:latest')
